...
class SelectPatternTest < Minitest::Test
  ...
  def test_pick_words_ending_in_e
    ...
    selected = []
    words.each do |x|
      selected.push(x) if x.end_with?('e')
    end
  ...
  def test_pick_words_ending_in_ing
    ...
    selected = []
    words.each do |x|
      selected.push(x) if x.end_with?("ing")
    end
  ...
  def test_pick_words_containing_e
    ...
    selected = []
    words.each do |x|
      selected.push(x) if x.include?('e')
    end
  ...
  def test_pick_dinosaurs
    ...
    dinosaurs = []
    animals.each do |x|
      dinosaurs.push(x) if x.include?('saurus')
    end
  ...
  def test_pick_floats
    ...
    floats = []
    numbers.each do |x|
      floats.push(x) if x.integer? == false
    end
  ...
  def test_pick_arrays
    ...
    arrays = []
    elements.each do |x|
      arrays.push(x) if x.class == Array
    end
