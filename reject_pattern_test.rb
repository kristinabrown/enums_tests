...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_vowels
    ...
    letters.each do |letter|
      remaining.push(letter) unless letter[/[aeiouy]/]
    end
  ...
  def test_remove_words_with_more_than_three_letters
    ...
    selected = []
    words.each do |x|
      selected.push(x) unless x.length > 3
    end
  ...
  def test_remove_words_ending_in_e
    ...
    selected = []
    words.each do |x|
      selected.push(x) unless x.end_with?('e')
    end
  ...
  def test_remove_words_ending_in_ing
    ...
    selected = []
    words.each do |word|
      selected.push(word) unless word.end_with?('ing')
    end
  ...
  def test_remove_words_containing_e
    ...
    selected = []
    words.each do |word|
      selected.push(word) unless word.include?('e')
    end
  ...
  def test_remove_dinosaurs
    ...
    animals.each do |animal|
      notasaurus.push(animal) unless animal.include?('saurus')
    end
  ...
  def test_remove_numbers
    ...
    elements.each do |element|
      not_numbers.push(element) if element.class == String
    end
  ...
  def test_remove_floats
    ...
    not_numbers = []
    elements.each do |x|
      not_numbers.push(x) unless x.class == Float
    end
  ...
  def test_remove_animals_starting_with_vowels
    ...
    remaining = []
    animals.each do |x|
      remaining.push(x) unless x.start_with?(x[/[aeiouy]/])
    end
  ...
  def test_remove_capitalized_words
    ...
    remaining = []
    words.each do |x|
      remaining.push(x) unless x == x.upcase
    end
  ...
  def test_remove_arrays
    ...
    remaining = []
    elements.each do |x|
      remaining.push(x) unless x.class == Array
    end
  ...
  def test_remove_hashes
    ...
    remaining = []
    elements.each do |x|
      remaining.push(x) unless x.class == Hash
    end
